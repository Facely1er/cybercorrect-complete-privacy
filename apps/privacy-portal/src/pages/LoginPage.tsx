...
export function LoginPage() {
  ...
  const { saveLocalUser } = useLocalUser();
  ...
  // No auth restriction: local login only
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create or update a local profile without external auth
    const result = await secureSubmit(async () => {
      const localId = `local-${sanitizedEmail.toLowerCase()}`;
      saveLocalUser({
        id: localId,
        name: sanitizedEmail.split('@')[0] || 'Local User',
        role: 'local_user',
        preferences: {}
      });
      return { user: { id: localId, email: sanitizedEmail } };
    }, { email: sanitizedEmail });

    if (result.success) {
      setSuccess('Signed in locally! Redirecting...');
      setTimeout(() => {
