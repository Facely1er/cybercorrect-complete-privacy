import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Card } from '../components/ui/Card';
import { Button } from '../components/ui/Button';
import { Badge } from '../components/ui/Badge';
import { 
  User, 
  Briefcase, 
  Users, 
  Shield, 
  ArrowRight,
  CheckCircle,
  Clock
} from 'lucide-react';
import { useBrand } from '../hooks/useBrand';
import { useLocalUser } from '../hooks/useLocalUser';
import { usePersona } from '../hooks/usePersona';
import { logger } from '../utils/logger';

interface PersonaOption {
  id: string;
  name: string;
  displayName: string;
  description: string;
  icon: React.ComponentType<{ className?: string }>;
  color: string;
  estimatedTime: string;
  features: string[];
  regulations: string[];
}

const personaOptions: PersonaOption[] = [
  {
    id: 'worker',
    name: 'Current Employee',
    displayName: 'Current Employee',
    description: 'I am an active employee who wants to exercise my privacy rights and manage my employment data.',
    icon: User,
    color: 'blue',
    estimatedTime: '5-10 min',
    features: [
      'Access personal employment data',
      'Request data corrections',
      'Manage privacy preferences',
      'Track data rights requests'
    ],
    regulations: ['ADA', 'CCPA', 'GDPR', 'EEOC']
  },
  {
    id: 'job_prospect',
    name: 'Job Applicant',
    displayName: 'Job Applicant',
    description: 'I am applying for a job and want to understand my privacy rights during the application process.',
    icon: Briefcase,
    color: 'green',
    estimatedTime: '3-5 min',
    features: [
      'Understand application privacy rights',
      'Request access to application data',
      'Learn about privacy protections'
    ],
    regulations: ['ADA', 'EEOC', 'CCPA']
  },
  {
    id: 'hr_staff',
    name: 'HR Staff',
    displayName: 'HR Staff',
    description: 'I work in human resources and need to manage employee data and privacy compliance.',
    icon: Users,
    color: 'purple',
    estimatedTime: '10-15 min',
    features: [
      'Process data rights requests',
      'Manage employee consent',
      'Track compliance metrics',
      'Handle privacy incidents'
    ],
    regulations: ['ADA', 'EEOC', 'CCPA', 'GDPR']
  },
  {
    id: 'dpo',
    name: 'Privacy Officer',
    displayName: 'Privacy Officer',
    description: 'I am responsible for privacy compliance oversight and program management.',
    icon: Shield,
    color: 'red',
    estimatedTime: '15-20 min',
    features: [
      'Comprehensive compliance monitoring',
      'Privacy program management',
      'Risk assessment and analytics',
      'Stakeholder oversight'
    ],
    regulations: ['All Applicable']
  }
];

export function PersonaSelectionPage() {
  const [selectedPersona, setSelectedPersona] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate();
  const { brand } = useBrand();
  const { localUser, updateLocalUser } = useLocalUser();
  const { hasCompletedOnboarding, setSelectedPersona: setPersonaInContext } = usePersona();

  const handlePersonaSelect = (personaId: string) => {
    setSelectedPersona(personaId);
  };

  const handleContinue = async () => {
    if (!selectedPersona) return;

    setIsLoading(true);

    try {
      // Update user's persona preference
      if (localUser) {
        updateLocalUser({ 
          preferences: { 
            ...localUser.preferences, 
            selectedPersona: selectedPersona 
          } 
        });
      }

      // Set persona in context
      setPersonaInContext(selectedPersona);
      
      // Check if user has completed onboarding for this persona
      if (hasCompletedOnboarding(selectedPersona)) {
        // Skip onboarding and go directly to dashboard
        navigate(`/persona/${selectedPersona}/dashboard`);
      } else {
        // Start onboarding process
        navigate(`/persona/${selectedPersona}/onboarding`);
      }
    } catch (error) {
      logger.error('Error selecting persona', error, {
        component: 'PersonaSelectionPage',
        operation: 'handlePersonaSelect',
        personaId: selectedPersona
      });
    } finally {
      setIsLoading(false);
    }
  };

  const getPersonaIcon = (persona: PersonaOption) => {
    const IconComponent = persona.icon;
    return <IconComponent className={`h-8 w-8 text-${persona.color}-600`} />;
  };

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          {/* Header */}
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold mb-4">
              Welcome to {brand.productNameWithTM}
            </h1>
            <p className="text-xl text-muted-foreground mb-6">
              Please select your role to get started with personalized privacy tools and guidance.
            </p>
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg max-w-2xl mx-auto">
              <p className="text-sm text-blue-700 dark:text-blue-300">
                <strong>Your privacy matters.</strong> This portal provides role-specific tools to help you understand 
                and exercise your privacy rights in the workplace.
              </p>
            </div>
          </div>

          {/* Persona Selection Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {personaOptions.map((persona) => (
              <Card 
                key={persona.id}
                className={`p-6 cursor-pointer transition-all duration-200 hover:shadow-lg ${
                  selectedPersona === persona.id 
                    ? 'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/20' 
                    : 'hover:bg-gray-50 dark:hover:bg-gray-800'
                }`}
                onClick={() => handlePersonaSelect(persona.id)}
              >
                <div className="flex items-start space-x-4">
                  <div className={`p-3 rounded-lg bg-${persona.color}-100 dark:bg-${persona.color}-900/30`}>
                    {getPersonaIcon(persona)}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-lg font-semibold">{persona.displayName}</h3>
                      <Badge variant="secondary" className="text-xs">
                        <Clock className="h-3 w-3 mr-1" />
                        {persona.estimatedTime}
                      </Badge>
                    </div>
                    <p className="text-muted-foreground mb-4 text-sm">
                      {persona.description}
                    </p>
                    
                    {/* Features */}
                    <div className="mb-4">
                      <h4 className="text-sm font-medium mb-2">What you can do:</h4>
                      <ul className="text-xs text-muted-foreground space-y-1">
                        {persona.features.map((feature, index) => (
                          <li key={index} className="flex items-center">
                            <CheckCircle className="h-3 w-3 text-green-500 mr-2 flex-shrink-0" />
                            {feature}
                          </li>
                        ))}
                      </ul>
                    </div>

                    {/* Regulations */}
                    <div className="flex flex-wrap gap-1">
                      {persona.regulations.map((regulation) => (
                        <Badge key={regulation} variant="secondary" className="text-xs">
                          {regulation}
                        </Badge>
                      ))}
                    </div>
                  </div>
                </div>

                {/* Selection Indicator */}
                {selectedPersona === persona.id && (
                  <div className="absolute top-4 right-4">
                    <div className="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center">
                      <CheckCircle className="h-4 w-4 text-white" />
                    </div>
                  </div>
                )}
              </Card>
            ))}
          </div>

          {/* Continue Button */}
          <div className="text-center">
            <Button 
              onClick={handleContinue}
              disabled={!selectedPersona || isLoading}
              size="lg"
              className="px-8"
            >
              {isLoading ? (
                <>
                  <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                  Setting up your experience...
                </>
              ) : (
                <>
                  Continue to {selectedPersona ? personaOptions.find(p => p.id === selectedPersona)?.displayName : 'Dashboard'}
                  <ArrowRight className="h-4 w-4 ml-2" />
                </>
              )}
            </Button>
            
            {selectedPersona && (
              <p className="text-sm text-muted-foreground mt-4">
                You can change your role later in your profile settings.
              </p>
            )}
          </div>

          {/* Help Section */}
          <div className="mt-12 bg-gray-100 dark:bg-gray-800 p-6 rounded-lg">
            <h3 className="text-lg font-semibold mb-4">Need Help Choosing?</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <h4 className="font-medium mb-2">For Employees & Job Applicants:</h4>
                <p className="text-muted-foreground">
                  Choose <strong>Current Employee</strong> if you're already working, or <strong>Job Applicant</strong> 
                  if you're applying for a position.
                </p>
              </div>
              <div>
                <h4 className="font-medium mb-2">For HR & Compliance Staff:</h4>
                <p className="text-muted-foreground">
                  Choose <strong>HR Staff</strong> for day-to-day privacy management, or <strong>Privacy Officer</strong> 
                  for comprehensive compliance oversight.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
