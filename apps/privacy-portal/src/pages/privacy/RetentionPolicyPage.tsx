...
export function RetentionPolicyPage() {
  ...
  const [policies] = useState<RetentionPolicy[]>(() => 
    localStorageService.getItem('retention_policies', [])
  );
  const [dataRecords] = useState<DataRecord[]>(() => 
    localStorageService.getItem('data_records', [])
  );
  ...
  const getRecordStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'scheduled_deletion':
        return <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">Scheduled Deletion</Badge>;
      case 'deleted':
        return <Badge className="bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300">Deleted</Badge>;
      case 'anonymized':
  ...
  const getSensitivityBadge = (sensitivity: string) => {
    switch (sensitivity) {
      ...
      default:
        return <Badge variant="general">{sensitivity}</Badge>;
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Retention Policy Management</h1>
        ...
        <TabsContent value="policies" className="space-y-6">
          ...
          <div className="space-y-4">
            {filteredPolicies.map((policy) => (
              <div key={policy.id} className="bg-white dark:bg-gray-900 rounded-lg border p-6">
                ...
                <div key={record.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {getSensitivityBadge(record.sensitivity)}
                    </div>
