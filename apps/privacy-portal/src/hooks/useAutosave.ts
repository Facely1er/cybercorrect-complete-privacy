...
export function useAutosave<T>(
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      const savedData = await localStorageService.getFormData(formId);
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(async () => {
      await localStorageService.saveFormData(formId, data);
      onSave?.(data);
    }, delay);
  ...
  const clearSaved = useCallback(async () => {
    await localStorageService.clearFormData(formId);
  }, [formId]);
