...
export function ComplianceAutomation({ organizationId }: ComplianceAutomationProps) {
  ...
  const [automationRules, setAutomationRules] = useState<AutomationRule[]>(() => [
    {
      id: 'auto-001',
      name: 'ADA Compliance Notice Reminder',
      description: 'Automatically send reminders for ADA employment compliance notices',
      type: 'notification',
      trigger: '90 days before deadline',
      actions: ['Send email to HR staff', 'Create compliance task', 'Update dashboard'],
      enabled: true,
      lastRun: '2024-05-15',
      nextRun: '2025-05-15',
      runCount: 12,
    ...
    {
      id: 'auto-004',
      name: 'Employee Consent Renewal Reminders',
      description: 'Remind employees about expiring privacy consent forms',
      type: 'notification',
  ...
  const handleToggleRule = (ruleId: string) => {
    setAutomationRules(prev => prev
      .filter(rule => rule != null) // Remove any null/undefined rules
      .map(rule => 
  ...
  const automationTemplates = [
    {
      name: 'EEOC Compliance Monitoring',
      description: 'Monitor and alert on EEOC compliance for workplace technology',
      type: 'workflow',
      trigger: 'New workplace technology integration'
    },
    {
      name: 'CCPA Employee Data Rights',
      description: 'Automated handling of CCPA employee data rights requests',
      type: 'workflow',
      trigger: 'CCPA request received'
    },
    {
    ...
    {
      name: 'Workplace Privacy Training Reminders',
      description: 'Send automated reminders for mandatory workplace privacy training',
      type: 'notification',
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">
              {cleanAutomationRules.filter(r => r.enabled).length}
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
              {cleanAutomationRules.reduce((sum, rule) => sum + (rule?.runCount || 0), 0)}
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
              {Math.round(cleanAutomationRules.reduce((sum, rule) => sum + (rule?.successRate || 0), 0) / cleanAutomationRules.length)}%
            </div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className={`${
                        ...
                        {rule.successRate}% ({(rule?.runCount || 0)} runs)
                      </div>
                    </div>
                  </div>
                </div>
