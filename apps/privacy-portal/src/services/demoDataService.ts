    ...
    const sampleDataRightsRequests = [
      {
        ...
        employeeIdentifier: 'Employee ID: 12345',
        employeeName: 'Sample Employee',
        description: 'Request to access complete employment records for review',
        status: 'submitted',
    ...
    const sampleComplianceObligations = [
      {
        ...
        description: 'Distribute annual ADA notification to all eligible employees and families',
        regulation: 'ferpa',
        priority: 'high',
        status: 'pending',
  ...
  importData: async (file: File) => {
    try {
      ...
      if (data.dataRightsRequests && Array.isArray(data.dataRightsRequests)) {
        data.dataRightsRequests.forEach((request: unknown) => {
      ...
      if (data.privacyIncidents && Array.isArray(data.privacyIncidents)) {
        data.privacyIncidents.forEach((incident: unknown) => {
      ...
      if (data.vendorAssessments && Array.isArray(data.vendorAssessments)) {
        data.vendorAssessments.forEach((vendor: unknown) => {
      ...
      if (data.consentRecords && Array.isArray(data.consentRecords)) {
        data.consentRecords.forEach((consent: unknown) => {
      ...
      if (data.complianceObligations && Array.isArray(data.complianceObligations)) {
        data.complianceObligations.forEach((obligation: unknown) => {
