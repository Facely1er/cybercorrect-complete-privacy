...
const ComplianceGapAnalyzer: React.FC = () => {
  ...
  const frameworks: Record<string, Framework> = useMemo(() => ({
    ...
    'nist_privacy_framework': {
      ...
      totalControls: 109,
      description: 'A tool for improving privacy through enterprise risk management, aligned with NIST CSF 2.0. Includes AI and privacy risk management guidance.',
      industry: 'All Industries',
      complexity: 'Intermediate',
      maturityLevels: ['Partial', 'Risk Informed', 'Repeatable', 'Adaptive'],
  ...
  const generateComplianceData = useCallback((): Record<string, DomainData> => {
    ...
    framework.domains.forEach((domain) => {
      const sectionScore = assessmentResults?.sectionScores?.find((s: { title: string; percentage: number }) => 
  ...
  const generateControlDescription = (domain?: string): string => {
    ...
    // Use domain to determine type if provided
    if (domain) {
      const domainLower = domain.toLowerCase();
      if (domainLower.includes('govern') || domainLower.includes('identify')) {
        return descriptions['governance'];
      } else if (domainLower.includes('protect') || domainLower.includes('detect')) {
        return descriptions['technical'];
      } else if (domainLower.includes('respond') || domainLower.includes('recover')) {
        return descriptions['operational'];
      }
    }
  ...
  const calculateRemediationEffort = (status: Control['status'], priority: Control['priority']): number => {
    const baseEffort: Record<string, Record<string, number>> = {
      'not_implemented': { critical: 45, high: 30, medium: 20, low: 10 },
      'partially_implemented': { critical: 25, high: 15, medium: 10, low: 5 },
      'planned': { critical: 15, high: 10, medium: 5, low: 3 }
    };
    return baseEffort[status]?.[priority] || 10;
  };

  const calculateEstimatedCost = (_status: Control['status'], priority: Control['priority']): number => {
    return generateCostEstimate(priority);
  };
        ...
        data?.controls?.forEach(control => {
          if (control.status !== 'fully_implemented' && control.status !== 'not_applicable') {
            gaps.push({
              ...
              complianceRisk: calculateRiskLevel(control.status, control.priority),
              ...
              urgencyScore: (priorityLevels[control.priority]?.weight || 1) * (control.status === 'not_implemented' ? 2 : 1)
            });
          }
        });
      });
    ...
    const generateTrendData = (): TrendData[] => {
      ...
      for (let i = months - 1; i >= 0; i--) {
        ...
        trends.push({
          ...
          implemented: Math.floor(Math.random() * 15) + 5,
                ...
                <BarChart data={Object.entries(complianceData).map(([domain, data]) => ({
                  ...
                  <Tooltip 
                    formatter={(value: unknown, name: string) => {
                      const formattedValue = name === 'score' ? `${value}%` : String(value);
                      const label = name === 'score' ? 'Compliance Score' : name === 'gaps' ? 'Gap Count' : 'Implemented';
                      return [formattedValue, label] as [React.ReactNode, string];
                    }}
                    contentStyle={{ 
                ...
                <PieChart>
                  <Pie
                    data={Object.entries(implementationStatus).map(([status, config]) => {
                      ...
                      const statusConfig = config as { label: string; color: string };
                      return {
                  ...
                  >
                    {Object.entries(implementationStatus).map((entry, index) => {
                      const statusConfig = entry[1] as { color: string };
                      return <Cell key={`cell-${index}`} fill={statusConfig.color} />;
                    })}
                  </Pie>
                  <Tooltip />
                </PieChart>
