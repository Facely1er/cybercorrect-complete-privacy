...
async function checkFile(filePath: string, relativePath: string): Promise<Issue[]> {
  const issues: Issue[] = [];
...
async function checkDirectory(dirPath: string, relativePath: string): Promise<Issue[]> {
  const issues: Issue[] = [];
